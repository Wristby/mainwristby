    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-400 mt-1">Manage your collection of timepieces.</p>
        </div>
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 mt-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label>Brand</Label>
                  <Input {...form.register("brand")} className="bg-slate-950 border-slate-800" placeholder="Rolex" />
                  {form.formState.errors.brand && <p className="text-red-500 text-xs">{form.formState.errors.brand.message}</p>}
                </div>
                <div className="space-y-2">
                  <Label>Model</Label>
                  <Input {...form.register("model")} className="bg-slate-950 border-slate-800" placeholder="Submariner" />
                </div>
                <div className="space-y-2">
                  <Label>Reference Number</Label>
                  <Input {...form.register("referenceNumber")} className="bg-slate-950 border-slate-800" placeholder="124060" />
                </div>
                <div className="space-y-2">
                  <Label>Serial Number</Label>
                  <Input {...form.register("serialNumber")} className="bg-slate-950 border-slate-800" />
                </div>
                <div className="space-y-2">
                  <Label>Year</Label>
                  <Input type="number" {...form.register("year")} className="bg-slate-950 border-slate-800" placeholder="2023" />
                </div>
                 <div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  <Input type="number" {...form.register("purchasePrice")} className="bg-slate-950 border-slate-800" />
                </div>
                <div className="space-y-2">
                  <Label>Target Sell Price (Cents)</Label>
                  <Input type="number" {...form.register("targetSellPrice")} className="bg-slate-950 border-slate-800" />
                </div>
                <div className="space-y-2">
            ...
            <TableBody>
              {isLoading ? (
                ...
                filteredInventory?.map((item) => (
                  ...
                  >
                    <TableCell className="font-medium text-white group-hover:text-emerald-400">
                      <Link href={`/inventory/${item.id}`}>{item.brand}</Link>
                    </TableCell>
                    <TableCell className="text-slate-300">{item.model}</TableCell>
                    <TableCell className="text-slate-400 font-mono text-xs">{item.referenceNumber}</TableCell>
                    <TableCell className="text-slate-400">{item.year || "N/A"}</TableCell>
                    <TableCell className="text-slate-400">{item.condition}</TableCell>
                    <TableCell>
                      <Badge 
                        variant="outline" 
                        className={
